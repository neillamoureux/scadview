...
class LabelAtlas:
    def __init__(self, ctx: moderngl.Context):
        ...
        self._texture = ctx.texture((self._width, self._height), 1, self._bytes)
        self._texture.build_mipmaps()
        self._texture.filter = (moderngl.LINEAR_MIPMAP_LINEAR, moderngl.LINEAR)

    def get_uv(self, char: str) -> np.ndarray:
    ...
    def _create_label_atlas(self) -> tuple[np.ndarray, dict[str, np.ndarray]]:
        ...
        self._uv_data = {
            char: np.array(uv, dtype=float) for char, uv in self._uv_data.items()
        }
        self._bytes = np.array(self._image, dtype=np.uint8)
    ...
    def _draw_chars(self, chars: str, font: ImageFont) -> None:
        ...
        # Create a new image for the atlas
        self._image = Image.new("L", (self._width, self._height), color=BLACK)
        ...
        # save atlas image
        self._image.save("atlas_image.png")
